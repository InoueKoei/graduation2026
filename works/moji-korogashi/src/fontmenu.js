...
//
//   value     : 内部識別子（この制作では 'b:<index>' / 'l:<family>'）
//   label     : 表示名
//   family    : CSS で使うファミリー名。指定すると選択肢をその書体で描画する。
//   groupHead : true なら上に区切り線を引く（同梱書体とローカル書体の境目）
//
// ※ TsumamiJi から流用。この制作向けに 2 つ足してある:
//    - groupHead（区切り線）
//    - select() を公開（読めない書体を選ばれたとき、選択を元に戻すため）
...
export function createFontMenu({ button, current, list, onChange }) {
  ...
  function render() {
    ...
    for (const opt of options) {
      ...
      if (opt.groupHead) li.classList.add('is-group-head');
