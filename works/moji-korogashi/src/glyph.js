// 書体の読み込みと、1文字 → 密な輪郭リング（折れ線）への変換。
//
// experiments/TsumamiJi/src/glyph.js からの流用。そちらの経路は
//   opentype.js で書体を読む → glyph.getPath() → ベジェを折れ線化 → 弧長で等間隔リサンプル
// で、外周とカウンター（穴）をそれぞれ独立した閉リングとして返す。
//
// ★ TsumamiJi のヘッダにある警告は、この制作でもそのまま効いている
//   「works/moji-stack は文字を積むための形なので convexHull で 1 つの塊にしている。
//     それをやると『あ』の穴も凹みも消える」
//   ころがしでは穴が消えると、ボールがカウンターの中に入り込んでしまう。凸包は取らない。
//
// ★ TsumamiJi との違い
//   - 等間隔リサンプルの目的が変わった。あちらは「ばねの自然長を揃える」ためだったが、
//     ここでは「ボールが当たる壁の分解能」になる。CONFIG.nodeSpacing で細かくする。
//   - polygonArea と strokeWidthEstimate を export した（course.js の穴判定と、
//     ボールが通れるかの事前判定で使う）。
...
/**
 * 字画のおおよその線幅。2 * |面積| / 周長。
 * experiments/TsumamiJi/src/rubber.js:100 で使われている見積もりと同じ式。
 * 面積は符号つきで足すので、穴は自動的に差し引かれる。
 *
 * ボールの直径がこれを超えるとコースとして成立しないため、事前判定に使う。
 */
export function strokeWidthEstimate(rings) {
  let area = 0;
  let perimeter = 0;
  ...
  return (2 * Math.abs(area)) / perimeter;
}
