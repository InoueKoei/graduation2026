...
/**
 * 前の字を打ち終えてから踏みはじめるまでの間隔 → **字の幅**。
 *
 * 測るのは**踏んでいない時間**。短いほど細く、長いほど広い字になる。
 * 迷い・探り・立ち止まりが、そのまま字幅として残る。
 *
 * 「その字を出すのにかかった時間」ではないので、「お」（5回踏む）も「あ」（1回）も、
 * 迷わず踏めば同じ幅になる。行の位置ではなく、ためらいを出すための値。
 *
 * Generative Gestaltung P_3_1_1_01 と同じ考え方（あちらは打鍵と打鍵の間隔を
 * 0〜5000ms → 文字サイズ 15〜800px に写している）。こちらは足なので下限を上げ、
 * 変化させるのは大きさではなく**字の幅**にしてある。
 * 組版の行が揃ったまま、字そのものの姿だけが平たくなったり細くなったりする。
 *
 * minMs を 0 ではなく 200 にしてあるのは、マットは面から面へ足を運ぶだけで
 * 400ms 前後かかるため。0 を基準にすると、普通に踏んでいるだけの字まで中くらいの幅になる。
 */
export const TIME_WIDTH = Object.freeze({
  minMs: 200,    // これ以下は最も細い。面から面へ運ぶだけならこのあたり
  maxMs: 2500,   // これ以上は最も広い。足で2.5秒止まればもう十分「迷っている」
  /**
   * 字の横幅（％）。100 が素の幅で、50 なら半分に痩せ、150 なら1.5倍に太る。
   *
   * 可変フォントの幅軸（wdth）ではなく、字を横に引き伸ばす／縮める。
   * 軸を持つフォントを要求しないので**どのフォントでも効く**——展示先の端末に
   * 何が入っていようと、幅が黙って効かなくなることがない。
   * 引き伸ばした字は字面が崩れるが、ここではそれ自体が「迷った跡」なので構わない。
   */
  minPct: 50,
  maxPct: 150,
 ...
 *
 *   size  … **そろうほど大きい**。ずれるほど縮む
 *   blur  … ずれるほどぼける（既定）
 *   slice … 字を**横の短冊**に切り、ずれるほど一本ずつ左右にずらす
 *
 * blur と slice は「ずれ＝崩れ」だが、size は向きが逆で**そろったことが得になる**。
 * 前者は崩さないように踏む動機に、後者は合わせにいく動機になる——
 * どちらが体験として効くかは踏んでみないと分からないので、両方を置いて選べるようにした。
 *
 * 短冊は既存作 works/Sessan の renderer.js と同じ仕掛け。
 * あちらは縦に切って上下へ、心拍の位相でずらしていた。
 * こちらは横に切って左右へ、**二人のずれ**で振幅を決める。
 */
export const SHIFT_MODES = Object.freeze({ size: '大きさ', blur: 'ぼかし', slice: '短冊' });
export const DEFAULT_SHIFT_MODE = 'blur';

/**
 * ずれ → 字の大きさ（em。1 が素の大きさ）。
 * ぴたりと合えば maxEm まで育ち、許容いっぱいまでずれると minEm まで縮む。
 *
 * 一人モードの TIME_WIDTH と違って**大きさ**を動かすので、行の高さが揃わなくなる。
 * それで構わない——二人で踏んだ跡が、行の乱れとして残るほうがこの場合は良い。
 *
 * **curve が要る理由。** ずれ ÷ pairMs をそのまま大きさに写すと、差が出ない。
 * pairMs は 1500ms あるが、二人で狙って踏めば実際のずれは 50〜500ms に固まる——
 * つまり比は 0.03〜0.33 の範囲しか使われず、どの字もほぼ maxEm で出てしまう。
 * 「少しずれた」と「大きくずれた」が同じ大きさに見えるのはこれが理由。
 *
 * そこで比を curve 乗してから写す。1 未満だと**ずれの小さいところが引き伸ばされる**ので、
 * 実際に起きる 50〜500ms のあいだで大きさがはっきり割れる。
 * 1 にすると元の比例に戻り、小さくするほど「ぴたり合ったときだけ大きい」に寄る。
 *
 *   ずれ  0ms → 2.00倍   50ms → 1.66倍   150ms → 1.43倍
 *        300ms → 1.22倍  600ms → 0.94倍  1000ms → 0.67倍  1500ms → 0.40倍
 *
 * 幅（DUO_WIDTH）とぼかし・短冊は今のところ比例のまま。同じ curve を掛けても良いが、
 * あちらは「崩れ」なので、少しのずれで大きく崩れると読めなくなる。
 */
export const DUO_SIZE = Object.freeze({
  maxEm: 2.0,
  minEm: 0.4,
  /** ずれの比に掛ける指数。1 = 比例、小さいほど小さなずれに敏感 */
  curve: 0.45,
});
 ...
 * ずれ → 字の横幅（％。100 が素の幅）。
 *
 * **これは見せ方の4つ目ではなく、上の3つのどれにも重ねられる入／切**。
 * 幅は大きさ・ぼかし・短冊のどれとも取り合いにならない（TIME_WIDTH と同じ
 * scaleX で伸縮させるだけなので、行の高さも崩さない）ので、独立させてある。
 * 合えば太く、ずれるほど痩せる。既定は切。
 */
export const DUO_WIDTH = Object.freeze({ maxPct: 150, minPct: 50 });
export const DEFAULT_DUO_WIDTH = false;
